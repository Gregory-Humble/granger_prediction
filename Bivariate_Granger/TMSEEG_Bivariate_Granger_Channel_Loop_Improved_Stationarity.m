...
saveDir = 'E:\Bivariate_Granger_Loop_Trial\Granger_Causality_TMS_EEG\Model_Order_10';
...
% '333', '335', '336', '338', '341', '343', '345', '346', '347',...
% '348', '349', '351'};  
...
% EEG = pop_loadset('filename', [IDlist{thisID} '_TMSEEG_BL_' Regions{Reg} '_ds_ica1_filt_ica2_clean_reref.set'],'filepath','E:\Alz_Clinical_Trial\Alz_Data_Analysis_10JAN20\TMSEEG\TMSEEG_ALZ_clean_2\BL\leftpfc');
...
%   for thisID = 1:numel(IDlist);
%      for Reg = 1:numel(Regions);
% EEG = pop_loadset('filename', [IDlist{thisID} '_TMSEEG_BL_' Regions{Reg} '_ds_ica1_filt_ica2_clean_reref.set'],'filepath','E:\Alz_Clinical_Trial\Alz_Data_Analysis_10JAN20\TMSEEG\TMSEEG_CONTROL_clean_2\BL\leftpfc');
% EEG = eeg_checkset(EEG);
% save([IDlist{thisID} '_TMSEEG_BL_' Regions{Reg} '_ds_ica1_filt_ica2_clean_reref.mat'], 'EEG');
%      end
%   end
...
EEG.data = EEG.data(:,1001:2000,:); % had to be changed for the control data as the time vector ends at 2000 (not 2001)
...
EEG.times = EEG.times(1001:2000); % had to be changed for the control data as the time vector ends at 2000 (not 2001)
